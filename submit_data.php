...
else
{
  ...
  $uquery =  mysqli_query($con,"UPDATE `userdetail` SET `fname` = '$fname',`mname` = '$mname',`lname` = '$lname',`conginitial` = '$conginitial',`designation` = '$designation',`hname` = '$hname',`gender` = '$gender',`oname` = '$oname',`congname` = '$congname',`cmobile` = '$cmobile',`email` = '$email',`aphone` = '$phone',`nationality` = '$nationality',`cstate` = '$cstate',`cdistrict` = '$cdistrict',`ccity` = '$ccity',`caddress` = '$caddress',`cpin` = '$cpin' WHERE `uid` = '$uid'");
  if($uquery)
  {
